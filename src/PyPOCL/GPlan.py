...
from PyPOCL.Flaws import FlawLib, OPF, TCLF, UGSV, UGGV, UGPV
...
class GPlan:
	...
	def insert_primitive(self, new_step: Operator):
		...
		for rc in new_step.path_reach_constraints:
			self.variableBindings.add_path_reach_constraint(rc[0], rc[1])
		...
		for arg in new_step.Args:
			...
			elif self.variableBindings.is_type(arg, 'path'):
				self.flaws.insert(self, UGPV(arg))
			else:
				print(f"argument {arg} of unknown type {arg.typ}")
				raise
